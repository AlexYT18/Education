...
int main(int argc, char** argv)
{
    ...
    else if (argc == 4 && strcmp(argv[2], "get") == 0)
    {
        ...
        return run_get(dst, outfilename);
    }
    else
    {
        fprintf(stderr,
            "Usage:\n"
            "  %s [IP:Port] [file.txt]  ->  PUT mode\n"
            "  %s [IP:Port] get [out.txt]   ->  GET mode\n",
            argv[0], argv[0]);
        return 1;
    }
}
...
int run_get(const char* dst, const char* outfilename)
{
    ...
    while (1)
    {
        ...
        int r = recv(sockfd, &idx_n, 4, MSG_WAITALL);
        if (r <= 0) break;
        recv(sockfd, &AA_n, 2, MSG_WAITALL);
        recv(sockfd, &BBB_n, 4, MSG_WAITALL);
        recv(sockfd, &hh, 1, MSG_WAITALL);
        recv(sockfd, &mm, 1, MSG_WAITALL);
        recv(sockfd, &ss, 1, MSG_WAITALL);
        recv(sockfd, &msglen_n, 4, MSG_WAITALL);
